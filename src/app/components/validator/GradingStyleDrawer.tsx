...
const METHODS = [
  {
    ...
    preview: (active: boolean) => (
      <div style={{ width: 64, height: 40, background: 'var(--background)', borderRadius: 8, padding: 4, display: 'flex', gap: 2, alignItems: 'center' }}>
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} style={{ flex: 1, height: 16, borderRadius: 2, background: i === 4 ? 'var(--color-success)' : 'var(--card-border)', opacity: i === 4 ? 1 : 0.4 }} />
        ))}
      </div>
    )
  },
  {
    ...
    preview: (active: boolean) => (
      <div style={{ width: 64, height: 40, background: 'var(--background)', borderRadius: 8, padding: 4, display: 'flex', flexDirection: 'column', gap: 2, justifyContent: 'center' }}>
        {[1, 2, 3].map(i => (
          <div key={i} style={{ width: '100%', height: 8, borderRadius: 4, background: i === 1 ? '#C0392B' : 'var(--card-border)', opacity: i === 1 ? 1 : 0.4 }} />
        ))}
      </div>
    )
  },
  {
    ...
    preview: (active: boolean) => (
      <div style={{ width: 64, height: 40, background: 'var(--background)', borderRadius: 8, padding: '8px 4px 4px', display: 'flex', gap: 2, alignItems: 'flex-end', justifyContent: 'center' }}>
        {[1, 2, 3, 4, 5].map(i => {
          const y = [6, 2, 0, 2, 6][i-1];
          return (
            <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: i === 5 ? '#1E6B40' : 'var(--card-border)', transform: `translateY(${-y}px)`, opacity: i === 5 ? 1 : 0.4 }} />
          );
        })}
      </div>
    )
  },
  {
    ...
    preview: (active: boolean) => (
      <div style={{ width: 64, height: 40, background: 'var(--background)', borderRadius: 8, padding: 4, display: 'flex', gap: 1, alignItems: 'flex-end' }}>
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} style={{ flex: 1, height: 24, borderRadius: 2, background: '#FFFFFF', border: '1px solid #E8EDF3', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--card-border)' }} />
          </div>
        ))}
      </div>
    )
  },
  {
    ...
    preview: (active: boolean) => (
      <div style={{ width: 64, height: 40, background: 'var(--background)', borderRadius: 8, padding: 4, display: 'flex', flexDirection: 'column', gap: 2 }}>
        <div style={{ display: 'flex', gap: 2 }}>
          <div style={{ flex: 1, height: 16, borderRadius: 4, background: 'var(--status-error-bg)' }} />
          <div style={{ flex: 1, height: 16, borderRadius: 4, background: '#ECFDF5' }} />
        </div>
        <div style={{ width: '100%', height: 8, borderRadius: 4, background: 'var(--card-border)' }} />
      </div>
    )
  }
];
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: isSelected ? '2px solid var(--accent-primary)' : '1px solid #E8EDF3',
                      display: 'flex',
                      alignItems: 'center',
                      gap: 16,
                      cursor: 'pointer',
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', lineHeight: 1.4 }}>
                        {method.description}
                      </p>
                    </div>
                    
                    {isSelected && (
                      <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[var(--accent-primary)] flex items-center justify-center">
